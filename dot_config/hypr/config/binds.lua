...
-- Left/Right cycle to the next/previous existing workspace instead of moving the window (from hyprland.conf)
...
-- Move active window to a specific workspace (1-10)
for i = 1, 10 do
    local key = i % 10
...
hl.bind(mainMod .. " + R",          hl.dsp.exec_cmd("hyprpanel -q && hyprpanel"))
hl.bind(mainMod .. " + S",          hl.dsp.exec_cmd("systemctl suspend"))
hl.bind(mainMod .. " + plus",       hl.dsp.exec_cmd("rofi -show calc -modi calc -no-show-match -no-sort"))
hl.bind(mainMod .. " + B",          hl.dsp.exec_cmd("$HOME/.config/hypr/scripts/move_to_browser_workspace.sh"))
...
-- Focus on workspace number (1-10)
for i = 1, 10 do
    local key = i % 10
    hl.bind(mainMod .. " + " .. key, hl.dsp.focus({ workspace = i }))
end

-- Focus on workspace number, scoped to a monitor
-- Absolute
for i = 1, NUM_WPM do
    local key = i % 10
